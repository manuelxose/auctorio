...
export function buildTextPrompt(input: TextPromptInput): TextPromptOutput {
  ...
  const briefOutline = Array.isArray(brief.outline)
    ? brief.outline
        .map((entry) => (entry && typeof entry === "object" ? String((entry as Record<string, unknown>).heading ?? "").trim() : ""))
        .filter(Boolean)
    : [];
  const briefInternalLinks = Array.isArray(brief.suggestedInternalLinks)
    ? brief.suggestedInternalLinks.filter((entry): entry is string => typeof entry === "string")
    : [];
  const briefFaqs = Array.isArray(brief.faqCandidates)
    ? brief.faqCandidates
        .filter((entry) => entry && typeof entry === "object")
        .map((entry) => entry as { question?: string; answer?: string })
  ...
  const briefQuestions = Array.isArray(brief.questionsToAnswer)
    ? brief.questionsToAnswer.filter((entry): entry is string => typeof entry === "string")
    : [];
  const briefSchemaTypes = Array.isArray(brief.schemaTypes)
    ? brief.schemaTypes.filter((entry): entry is string => typeof entry === "string")
    : [];
  ...
  const briefSemanticKeywords = Array.isArray(brief.semanticKeywords)
    ? brief.semanticKeywords.filter((entry): entry is string => typeof entry === "string")
    : [];
  ...
  const briefInstructions: string[] = [];
  if (intentGuidance) {
  ...
  if (briefInternalLinks.length > 0) {
    briefInstructions.push(
      `Internal links (verified destination URLs — use ONLY these, with natural anchors): ${briefInternalLinks.join(", ")}.`,
    );
  }
  if (briefFaqs.length > 0) {
    briefInstructions.push(
      `End with an FAQ section using these question/answer pairs: ${briefFaqs.map((faq) => `Q: ${faq.question}`).join(" | ")}.`,
    );
  }
  ...
  briefInstructions.push(
    "Formatting requirements: semantic HTML only (h2/h3, strong for genuinely important concepts, ul/ol, tables when useful). " +
