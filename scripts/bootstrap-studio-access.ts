import { inviteStudioUser, resolveTenantBySlug } from "../src/studio/auth";
...
async function main(): Promise<void> {
  for (const workspace of WORKSPACE_BOOTSTRAP) {
    const tenant = await resolveTenantBySlug(workspace.slug);
    if (!tenant) {
    ...
    for (const user of workspace.users) {
      ...
      console.log(
        `  invited ${invitation.email} -> ${workspace.slug} (${user.roleKeys.join(",")})`
      );
    }
  }
}
